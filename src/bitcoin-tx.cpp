...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " gopecoin-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  gopecoin-tx [options] <hex-tx> [commands]  Update hex-encoded gopecoin transaction\n"
                "or:     gopecoin-tx [options] -create [commands]   Create hex-encoded gopecoin transaction\n"
                "\n";
